...
class NewEquipTableViewController: UITableViewController {
    ...
    func saveEquip(_ sender:AnyObject) {
        ...
        let _ = EquipFileControl.sharedInstance().addEquipInfoToFile(0, XMLID: equip.xmlFile.id, XMLName: equip.xmlFile.name as String, imageSet: NSMutableArray(), path: "\(equip.xmlFile.id)", groupID: EquipManager.sharedInstance().defaultGroupId, status: FileSystem.Status.new.rawValue | FileSystem.Status.download.rawValue);
        print("saveEquip");
        self.backPressed();
    }

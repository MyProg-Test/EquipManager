...
class EquipImagePickerTableViewController: UITableViewController,UIAlertViewDelegate,UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        if((lastChosen.object(forKey: UIImagePickerControllerMediaType) as! NSString) == (kUTTypeImage as NSString)){
            ...
            let equipIndex = DetailEquipViewController.data_source!.equipIndex;
            let dictInfo = EquipFileControl.sharedInstance().fs!.equipArray.object(at: equipIndex) as! NSMutableDictionary;
            
            _ = EquipFileControl.sharedInstance().addImageInfoToFile(equipIndex, imageID: (imageName as NSString).integerValue, imagePath: dictInfo.object(forKey: FileSystem.equipKey.path) as! String, imageName: "\(imageName).png", status: FileSystem.Status.new.rawValue | FileSystem.Status.download.rawValue);
            let chosenImageData: Data = UIImagePNGRepresentation(chosenImage)!;
            try! chosenImageData.write(to: EquipFileControl.sharedInstance().getImageFilePathFromFile(equipIndex, imageIndex: DetailEquipViewController.data_source!.imageInfo.historyImage.count)!);
            for i in stride(from: 0, to: EquipFileControl.sharedInstance().getImageCountFromFile(equipIndex), by: 1){
                if EquipFileControl.sharedInstance().isMainImageFromFile(equipIndex, imageIndex: i){
                    _ = EquipFileControl.sharedInstance().resetMainImageFromFile(equipIndex, imageIndex: i);
                }
            }
            _ = EquipFileControl.sharedInstance().setMainImageFromFile(equipIndex, imageIndex: DetailEquipViewController.data_source!.imageInfo.historyImage.count);
            DetailEquipViewController.data_source!.updateEquip(DetailEquipViewController.data_source!.equipIndex);
            self.tableView.reloadData();
        }

...
class FileSystem {
    ...
    func getImageCount(_ index:Int)->Int{
        let imageSet = self.getEquip(index)!.object(forKey: equipKey.imageSet) as! NSArray;
    ...
    func modifyImageName(_ equipIndex:Int, imageIndex:Int, name:String){
        objc_sync_enter(self);
        (((self.equipArray.object(at: equipIndex) as AnyObject).object(forKey: equipKey.imageSet) as! NSArray).object(at: imageIndex) as AnyObject).setValue(name, forKey: imageSetKey.imageName);
        objc_sync_exit(self);
    }
    //修改设备状态
    ...
    func modifyImageID(_ equipIndex: Int, imageIndex: Int, id: Int) {
        objc_sync_enter(self);
        (((self.equipArray.object(at: equipIndex) as AnyObject).object(forKey: equipKey.imageSet) as! NSArray).object(at: imageIndex) as AnyObject).setValue(id, forKey: imageSetKey.imageID);
        objc_sync_exit(self);
    }
    ...
    func modifyImageStatus(_ equipIndex:Int, imageIndex:Int, status:Int){
        objc_sync_enter(self);
        (((self.equipArray.object(at: equipIndex) as AnyObject).object(forKey: equipKey.imageSet) as! NSArray).object(at: imageIndex) as AnyObject).setValue(status, forKey: imageSetKey.status);
        objc_sync_exit(self);
    }
    //获取设备
    ...
    func setMainImage(_ equipIndex:Int, imageIndex:Int) -> Bool {
        if isMainImage(equipIndex, imageIndex: imageIndex){
            return true;
        }
        let name = getImageName(equipIndex, imageIndex: imageIndex);
        let parentID = getEquipParentID(equipIndex);
        modifyImageName(equipIndex, imageIndex: imageIndex, name: "\(parentID)_\(name)");
        return true;
    }
    
    func resetMainImage(_ equipIndex:Int, imageIndex:Int) -> Bool {
        if isMainImage(equipIndex, imageIndex: imageIndex){
            return true;
        }
        var name = getImageName(equipIndex, imageIndex: imageIndex);
        let parentID = getEquipParentID(equipIndex);
        name = name.replacingOccurrences(of: "\(parentID)_", with: "")
        modifyImageName(equipIndex, imageIndex: imageIndex, name: name);
        return true;
    }
    //从文件中读取
